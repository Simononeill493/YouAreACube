...
namespace IAmACube
{
    class SpriteBoxMatrix<TBox> : ContainerScreenItem where TBox : CubeSpriteBox
    {
        ...
        public SpriteBoxMatrix(IHasDrawLayer parent,int width,int height, int increment = DefaultItemsIncrement) : base(parent)
        {
            ...
            _width = width;
            _height = height;
            _increment = increment + SpriteManager.GetSpriteSize(MenuSprites.SpriteBox).X;
        }
        ...
        public void UpdateBoxLocations()
        {
            ...
            {
                _boxes[i].SetLocationConfig(new IntPoint(xOffset, yOffset), CoordinateMode.ParentPixel, centered: false);
                xOffset += _increment;
